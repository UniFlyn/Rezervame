...
```env
# .env
POSTMARK_API_KEY=<will be provided separately>
POSTMARK_FROM_EMAIL=noreply@rezervame.com
POSTMARK_REPLY_TO=soporte@rezervame.com
POSTMARK_MESSAGE_STREAM=outbound
```

> **IMPORTANT:** The API key will be sent in a separate message. Store it only in `.env` — never commit it to the repository.
...
| `password-reset` | Password reset request | User | userName, verificationCode (6-digit), resetCode, expiresIn |
...
Booking reminders run via a protected HTTP cron endpoint (BullMQ can be added later).

**Endpoint:** `POST /api/cron/booking-reminders`  
**Auth:** `Authorization: Bearer <CRON_SECRET>`  
**Render cron:** schedule hourly (e.g. `0 * * * *`).

Implementation: `Backend/src/email/booking-reminder.service.ts`

---

## 7b. Legacy BullMQ example (optional future)

Booking reminders can alternatively be processed via BullMQ scheduled jobs.
...
- [ ] Server API Token stored in Render / `Backend/.env` as `POSTMARK_API_KEY` (**never commit the token**)
- [x] `EmailModule` implemented in NestJS (`Backend/src/email/`)
- [x] `EmailService` with `sendWithTemplate` and `sendRaw` methods
- [ ] Templates created in Postmark dashboard (see catalog in §5) — aliases must match `email.constants.ts`
- [x] Webhook endpoint `POST /webhooks/postmark` (`email-webhook.controller.ts`)
- [ ] Webhook configured in Postmark dashboard pointing to production URL
- [x] Booking reminder cron (`booking-reminder.service.ts`, `POST /api/cron/booking-reminders`)
- [x] Business registration sends `welcome-business` template when Postmark is configured
...
```env
POSTMARK_API_KEY=<from Postmark → Server API tokens>
POSTMARK_FROM_EMAIL=noreply@rezervame.com
POSTMARK_REPLY_TO=soporte@rezervame.com
POSTMARK_MESSAGE_STREAM=outbound
```

Optional alerts stream sender: `notificaciones@rezervame.com` (configure as additional verified sender in Postmark if used).
