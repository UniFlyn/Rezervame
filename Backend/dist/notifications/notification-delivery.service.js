...
async function logDelivery(prisma, channel, recipient, subject, body, status, error, meta) {
    try {
...
async function sendEmailWithTemplate(prisma, options) {
    const recipient = options.to.trim();
    if (!recipient)
        return { ok: false, error: 'Missing recipient' };
    if (!(await (0, system_integration_config_1.resolvePostmarkConfig)(prisma))) {
        await logDelivery(prisma, 'email', recipient, options.templateAlias, JSON.stringify(options.templateModel).slice(0, 2000), 'skipped', 'Postmark not configured');
        return { ok: true, skipped: true };
    }
    try {
        const result = await (0, email_service_2.postmarkSendWithTemplate)(prisma, { ...options, to: recipient });
        if ('skipped' in result) {
            return { ok: true, skipped: true };
        }
        await logDelivery(prisma, 'email', recipient, options.templateAlias, JSON.stringify(options.templateModel).slice(0, 2000), 'sent', undefined, result.MessageID);
        return { ok: true, messageId: result.MessageID };
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        await logDelivery(prisma, 'email', recipient, options.templateAlias, JSON.stringify(options.templateModel).slice(0, 2000), 'failed', msg);
...
async function sendEmail(prisma, to, subject, html, text) {
    ...
    if (!recipient)
        return { ok: false, error: 'Missing recipient' };
    if (!(await (0, system_integration_config_1.resolvePostmarkConfig)(prisma))) {
        await logDelivery(prisma, 'email', recipient, subject, text || html, 'skipped', 'Postmark not configured');
        console.log(`[email:skipped] To: ${recipient} | ${subject}`);
        return { ok: true, skipped: true };
    }
    try {
        const result = await (0, email_service_1.postmarkSendRaw)(prisma, {
            to: recipient,
            subject,
            htmlBody: html,
            textBody: text,
...
async function sendSms(prisma, to, body) {
...
async function notifyAdminNewTicket(prisma, ticket) {
    const cfg = await loadMessagingConfig(prisma);
