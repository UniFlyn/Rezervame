/**
 * Seeds discoverable demo businesses + services for categories with no listings,
 * and adds audience-tagged services to Luxe Hair Studio (men / women / kids filters).
 *
 * Usage: cd Backend && node scripts/seed-category-businesses.mjs
 */
...
async function allocateMerchantNumber() {
  ...
  throw new Error('Could not allocate merchant number');
}

/** @type {Array<{ key: string; name: string; owner: string; email: string; address: string; description: string; lat: number; lng: number; bannerUrl: string; logoUrl: string; services: Array<{ name: string; category: string; duration: number; price: number }> }>} */
const CATEGORY_DEMOS = [
  ...
  },
  {
    ...
    lat: 8.9945,
    lng: -79.5128,
    bannerUrl: 'https://rezervame-assets-abs.s3.ap-southeast-2.amazonaws.com/uploads/defaults/categories/massage.jpg',
...
/** Extra services for Luxe Hair Studio audience filters */
const LUXE_EXTRA_SERVICES = [
...
async function ensureCategory(key, labels) {
  await prisma.category.upsert({
    where: { key },
    update: { active: true, ...labels },
    create: { key, active: true, sortOrder: 50, ...labels },
  });
}

async function upsertDemoBusiness(demo) {
  const gallery = [demo.bannerUrl, demo.logoUrl];
  const existing = await prisma.business.findUnique({ where: { email: demo.email } });
  ...
  if (existing) {
    ...
    console.log(`  updated business: ${demo.name}`);
  } else {
    business = await prisma.business.create({
      data: {
        ...businessData,
        email: demo.email,
        merchantNumber: await allocateMerchantNumber(),
      },
    });
    console.log(`  created business: ${demo.name}`);
  }
  ...
  const staffExists = await prisma.staff.findFirst({ where: { businessId: business.id } });
  if (!staffExists) {
    const services = await prisma.service.findMany({ where: { businessId: business.id }, take: 4 });
...
async function addLuxeServices() {
  const luxe = await prisma.business.findFirst({
    where: { OR: [{ email: 'owner@rezervame.com' }, { name: 'Luxe Hair Studio' }] },
  });
  if (!luxe) {
    console.log('Luxe Hair Studio not found — skipping extra services');
    return;
  }
  for (const svc of LUXE_EXTRA_SERVICES) {
    ...
    if (!found) {
      await prisma.service.create({ data: { ...svc, businessId: luxe.id } });
      console.log(`  + Luxe service: ${svc.name}`);
    }
  }
}

async function main() {
  console.log('Ensuring category records…');
  await ensureCategory('massage', {
    labelEn: 'Massage',
    labelEs: 'Masaje',
    sortOrder: 55,
    imageUrl: 'https://rezervame-assets-abs.s3.ap-southeast-2.amazonaws.com/uploads/defaults/categories/massage.jpg',
  });
  await ensureCategory('beautyService', {
    labelEn: 'Beauty',
    labelEs: 'Belleza',
    sortOrder: 40,
  ...
  console.log('Seeding demo businesses per category…');
  for (const demo of CATEGORY_DEMOS) {
    console.log(`\n[${demo.key}]`);
    await upsertDemoBusiness(demo);
  }

  console.log('\nAdding Luxe Hair Studio audience services…');
  await addLuxeServices();
