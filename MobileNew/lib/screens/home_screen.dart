...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget _buildTopServiceGridCard(HomeFeaturedItem f) {
    final title = f.serviceTitleKey.tr();
    final reviews = '(${f.reviewCount})';
    return Material(
      color: Colors.transparent,
      child: InkWell(
        onTap: () => _openVenueDetail(f.venueId, f.salonName, f.unsplashId, f.price, f.rating, '0'),
        borderRadius: BorderRadius.circular(16),
        child: Container(
          decoration: BoxDecoration(
            color: AppColors.white,
                ...
                children: [
                  ClipRRect(
                    ...
                    child: AspectRatio(
                      aspectRatio: 1.05,
                      child: ChainedNetworkImage(
                        urls: ChainedNetworkImage.urlsForUnsplashId(f.unsplashId, w: 500),
                        fit: BoxFit.cover,
                      ),
                    ),
                  ),
              ...
              return Column(
                children: [
                  CircleAvatar(
                    ...
                    backgroundImage: NetworkImage('https://i.pravatar.cc/200?u=${b.avatarSeed}'),
                  ),
    ...
    return Row(
      children: [
        const CircleAvatar(
          ...
          backgroundImage: NetworkImage('https://i.pravatar.cc/150?u=amalia'),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: Column(
  ...
  Widget _buildPromoBannerImage(HomePromoBannerItem item) {
    final networkUrl = 'https://images.unsplash.com/photo-${item.unsplashId}?q=80&w=600&fit=crop';
    final chain = ChainedNetworkImage.chainFrom(networkUrl, item.unsplashId, w: 600);
