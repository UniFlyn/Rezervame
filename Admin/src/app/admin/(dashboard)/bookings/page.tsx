      ...
      return [
        {
          label: "Approve",
          newStatus: "Approved",
          style: "bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-100",
          icon: <CheckCircle className="w-3.5 h-3.5" />,
        },
        {
          label: "Reject",
          newStatus: "Rejected",
          style: "bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-100",
      ...
      return [
        {
          label: "Complete",
          newStatus: "Completed",
          style: "bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-100",
          icon: <CheckCircle className="w-3.5 h-3.5" />,
        },
        {
          label: "Cancel",
          newStatus: "Cancelled",
          style: "bg-slate-50 text-slate-700 hover:bg-slate-100 border border-slate-200",
              ...
              {getActions(selectedBooking.status).length > 0 ? (
                <div className="flex flex-wrap gap-2 rounded-2xl border border-slate-100 bg-white p-4">
                  {getActions(selectedBooking.status).map((action) => {
                    const isUpdating = updatingId === selectedBooking.id;
                    return (
                      <button
                        key={action.newStatus}
                        type="button"
                        ...
                        className={cn(
                          "inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-[11px] font-bold transition-all shadow-sm",
                          action.style,
                          isUpdating && "cursor-not-allowed opacity-50",
                        )}
                      >
                        {isUpdating ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : action.icon}
                        {action.label}
                      </button>
                    );
                  })}
