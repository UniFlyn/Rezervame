...
function EventCard({
  ...
  return (
    <article className="group flex h-full min-w-0 flex-col overflow-hidden rounded-[24px] border border-white/60 bg-white shadow-lg shadow-slate-900/5 transition duration-500 hover:-translate-y-1 hover:shadow-2xl hover:shadow-[#ff5a5f]/15">
      <div className="relative aspect-[4/3] overflow-hidden bg-slate-200">
        ...
        <div className="absolute left-3 top-3">
          <span className="inline-flex items-center gap-1 rounded-full bg-[#ff5a5f] px-2.5 py-1 text-[8px] font-black uppercase tracking-[0.16em] text-white shadow-md">
        ...
        <div className="absolute right-3 top-3 flex min-w-[52px] flex-col items-center rounded-xl border border-white/20 bg-white/95 px-2 py-1.5 text-center shadow-lg backdrop-blur-md">
          <span className="text-[9px] font-black uppercase tracking-widest text-[#ff5a5f]">
            {when.month}
          </span>
          <span className="text-xl font-black leading-none text-slate-900">{when.day}</span>
          <span className="mt-0.5 text-[8px] font-bold text-slate-500">{when.time}</span>
        </div>
      ...
      <div className="flex flex-1 flex-col p-4">
        <p className="mb-3 line-clamp-2 min-h-[2.5rem] text-xs font-medium leading-relaxed text-slate-600">
          {event.body}
        </p>
        <div className="mb-3 flex flex-col gap-2 text-[10px] font-bold text-slate-500">
          <span className="inline-flex items-center gap-1.5 rounded-lg bg-slate-50 px-2 py-1.5">
            <Calendar size={12} className="shrink-0 text-[#ff5a5f]" aria-hidden />
            <span className="truncate">{when.full}</span>
          </span>
          {event.location ? (
            <span className="inline-flex items-center gap-1.5 rounded-lg bg-slate-50 px-2 py-1.5">
              <MapPin size={12} className="shrink-0 text-[#ff5a5f]" aria-hidden />
        ...
        <div className="mt-auto flex items-center justify-between gap-2 border-t border-slate-100 pt-3">
          ...
          {hasWebsite ? (
            <a
              ...
              className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-slate-900 px-3 py-2 text-[9px] font-black uppercase tracking-widest text-white transition hover:bg-[#ff5a5f]"
            >
...
export function HomeEventsSection() {
  ...
  return (
    <section className="relative mb-16 overflow-hidden rounded-[32px] border border-slate-900/5 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 p-6 sm:p-10 md:p-12">
      <div
        className="pointer-events-none absolute -right-20 -top-20 h-72 w-72 rounded-full bg-[#ff5a5f]/25 blur-[100px]"
        aria-hidden
      />
      <div
      ...
      <div className="relative z-10 mb-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-xl text-left">
          ...
          <p className="mt-2 font-medium text-slate-400">{t("homeEventsSub")}</p>
        </div>
        <Link
          href="/events"
          className="group inline-flex shrink-0 items-center self-start rounded-xl border border-white/15 bg-white/10 px-6 py-3 text-sm font-bold text-white backdrop-blur-sm transition hover:border-[#ff5a5f]/50 hover:bg-white/15 sm:self-auto"
        >
          {t("homeEventsViewAll")}
          <ArrowRight
            size={16}
