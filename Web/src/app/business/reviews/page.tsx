...
export default function ReviewsPage() {
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="rounded-[32px] border border-[var(--rz-gray-100)] bg-white p-10 shadow-xl shadow-[color:rgba(231,234,239,0.5)]">
        <div className="flex flex-col items-center justify-center gap-4 text-center md:flex-row md:justify-between md:text-left">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-[var(--rz-gray-500)]">Overall rating</p>
            <div className="mt-2 flex items-end gap-3"><span className="text-6xl font-black tracking-tighter text-[var(--rz-navy)]">{averageRating}</span><span className="pb-2 text-sm font-bold text-[var(--rz-gray-500)]">/ 5</span></div>
            <div className="mt-2 flex justify-center md:justify-start">{[1,2,3,4,5].map((s)=><Star key={s} size={24} className={clsx(s<=Math.round(Number(averageRating))?'fill-amber-400 text-amber-400':'text-[var(--rz-gray-200)]')} />)}</div>
          </div>
          <div className="w-full max-w-md space-y-3 md:max-w-lg">
            {[5,4,3,2,1].map((rating)=>{const count=groupedAllReviews.filter((r)=>r.rating===rating).length; const percentage=totalReviews?(count/totalReviews)*100:0; return <div key={rating} className="flex items-center gap-3"><span className="w-4 text-[10px] font-bold text-[var(--rz-gray-500)]">{rating}</span><div className="h-2 flex-1 overflow-hidden rounded-full bg-[var(--rz-gray-050)]"><div className="h-full rounded-full bg-amber-400" style={{width:`${percentage}%`}} /></div><span className="w-8 text-[10px] font-bold text-[var(--rz-gray-500)]">{count}</span></div>;})}
            <p className="text-center text-[10px] font-black uppercase tracking-widest text-[var(--rz-gray-500)] md:text-right">{totalReviews} total customer visits</p>
          </div>
        </div>
      </div>

      <BusinessFilterToolbar searchValue={search} onSearchChange={setSearch} searchPlaceholder="Search reviews...">
        <div className="flex items-center gap-2 rounded-2xl border border-[var(--rz-gray-100)] bg-[var(--rz-gray-050)] px-4 py-2"><select className="cursor-pointer bg-transparent text-[10px] font-black uppercase tracking-widest outline-none" value={filterRating==='all'?'all':String(filterRating)} onChange={(e)=>setFilterRating(e.target.value==='all'?'all':Number(e.target.value))}><option value="all">All stars</option><option value="5">5 stars</option><option value="4">4 stars</option><option value="3">3 stars</option><option value="2">2 stars</option><option value="1">1 star</option></select></div>
        <div className="flex items-center gap-2 rounded-2xl border border-[var(--rz-gray-100)] bg-[var(--rz-gray-050)] px-4 py-2"><MessageSquare size={14} className="text-[var(--rz-gray-500)]" /><select className="cursor-pointer bg-transparent text-[10px] font-black uppercase tracking-widest outline-none" value={filterStatus} onChange={(e)=>setFilterStatus(e.target.value as 'all'|'Pending'|'Replied')}><option value="all">All statuses</option><option value="Pending">Pending reply</option><option value="Replied">Replied</option></select></div>
      ...
      <div className="space-y-4 relative min-h-[200px]">
        ...
        {paginatedReviews.length===0 && !isLoading ? <div className="rounded-[32px] border-2 border-dashed border-[var(--rz-gray-100)] bg-white p-20 text-center"><MessageSquare size={48} className="mx-auto mb-4 text-[var(--rz-gray-200)]" /><p className="text-sm font-bold uppercase tracking-widest text-[var(--rz-gray-500)]">No reviews matched your filters</p></div> : paginatedReviews.map((review)=>(
          <div key={review.id} className="animate-in fade-in rounded-[32px] border border-[var(--rz-gray-100)] bg-white p-8 shadow-xl shadow-[color:rgba(231,234,239,0.5)] duration-500 slide-in-from-bottom-4">
            <div className="flex flex-col gap-6 md:flex-row">
              <div className="flex shrink-0 flex-col items-center"><div className="h-16 w-16 overflow-hidden rounded-[24px] border-4 border-[var(--rz-gray-050)] shadow-sm"><img src={review.avatar} alt={review.customerName} className="h-full w-full object-cover" /></div><div className="mt-3 rounded-full bg-emerald-50 px-3 py-1 text-[9px] font-black uppercase tracking-tighter text-emerald-600">Verified</div></div>
              <div className="min-w-0 flex-1 space-y-4">
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                  <div>
                    <h4 className="mb-1 text-lg font-black leading-none text-[var(--rz-navy-800)]">{review.customerName}</h4>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--rz-gray-500)]">{new Date(review.date).toLocaleDateString()} · {review.serviceName} with {review.staffName}</p>
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    <div className="flex items-center gap-1">
                      <span className="text-[9px] font-black uppercase text-[var(--rz-gray-300)] mr-1">Overall</span>
                      {[1,2,3,4,5].map((s)=><Star key={s} size={16} className={clsx(s<=review.rating?'fill-amber-400 text-amber-400':'text-[var(--rz-gray-100)]')} />)}
                    </div>
                    {review.staffRating && (
                      <div className="flex items-center gap-1">
                        <span className="text-[8px] font-black uppercase text-[var(--rz-gray-300)] mr-1">Staff</span>
                        {[1,2,3,4,5].map((s)=><Star key={s} size={12} className={clsx(s<=(review.staffRating||0)?'fill-blue-400 text-blue-400':'text-[var(--rz-gray-100)]')} />)}
                      </div>
                    )}
                    {review.businessRating && (
                      <div className="flex items-center gap-1">
                        <span className="text-[8px] font-black uppercase text-[var(--rz-gray-300)] mr-1">Venue</span>
                        {[1,2,3,4,5].map((s)=><Star key={s} size={12} className={clsx(s<=(review.businessRating||0)?'fill-purple-400 text-purple-400':'text-[var(--rz-gray-100)]')} />)}
                      </div>
                    )}
                  </div>
                </div>
                <p className="font-medium italic leading-relaxed text-[var(--rz-gray-600)]">&ldquo;{review.comment}&rdquo;</p>
                {review.reply ? <div className="relative mt-4 rounded-[24px] border border-[var(--rz-gray-100)] bg-[var(--rz-gray-050)] p-6"><div className="absolute -top-3 left-6 rounded-full bg-[var(--rz-navy)] px-3 py-1 text-[8px] font-black uppercase tracking-[0.2em] text-white shadow-lg">Your reply</div><p className="text-sm font-bold italic text-[var(--rz-gray-600)]">{review.reply}</p></div> : <div className="pt-2">{replyingTo===review.id ? <div className="animate-in zoom-in-95 space-y-4 duration-300 fade-in"><textarea value={replyText} onChange={(e)=>setReplyText(e.target.value)} placeholder="Write your response..." className="min-h-[120px] w-full rounded-[24px] border border-[var(--rz-gray-100)] bg-[var(--rz-gray-050)] p-6 text-sm font-bold text-[var(--rz-gray-700)] transition-all focus:outline-none focus:ring-4 focus:ring-primary/10" /><div className="flex justify-end gap-3"><button type="button" onClick={()=>setReplyingTo(null)} className="px-6 py-3 text-[10px] font-black uppercase tracking-widest text-[var(--rz-gray-500)] transition hover:text-[var(--rz-gray-600)]">Cancel</button><button type="button" onClick={()=>void handleReply(review.id)} className="rounded-2xl bg-[var(--rz-navy)] px-8 py-3 text-[10px] font-black uppercase tracking-widest text-white shadow-xl shadow-[color:var(--rz-gray-200)] transition hover:bg-primary">Send reply</button></div></div> : <button type="button" onClick={()=>setReplyingTo(review.id)} className="flex items-center gap-2 rounded-xl bg-primary/5 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-primary transition-transform hover:translate-x-1"><Reply size={14} /> Reply</button>}</div>}
              </div>
              <div className="shrink-0"><button type="button" onClick={()=>void handleDelete(review.id)} className="rounded-2xl p-3 text-[var(--rz-gray-200)] transition-colors hover:bg-rose-50 hover:text-rose-500"><Trash2 size={18} /></button></div>
            </div>
          </div>
        ))}
      </div>
