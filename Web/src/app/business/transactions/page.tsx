        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[960px] border-collapse text-left">
            ...
            <tbody className="divide-y divide-slate-50 text-sm">
              {paginatedTransactions.map((tx)=><tr key={tx.id} className="font-bold text-slate-600 transition-colors hover:bg-slate-50/80"><td className="px-6 py-4 font-mono text-xs text-slate-500">{tx.id}</td><td className="px-6 py-4 font-mono text-xs">{tx.bookingId||'—'}</td><td className="max-w-[180px] truncate px-6 py-4 text-xs">{tx.customerEmail||'—'}</td><td className="px-6 py-4 text-xs">{tx.staffMember||'—'}</td><td className="whitespace-nowrap px-6 py-4 text-xs">{new Date(tx.date).toLocaleString()}</td><td className="px-6 py-4"><div className={clsx('inline-flex items-center gap-1',tx.type==='Earning'?'text-emerald-600':'text-orange-600')}>{tx.type==='Earning'?<ArrowUpRight className="h-4 w-4" />:<ArrowDownLeft className="h-4 w-4" />}<span>{tx.type||'—'}</span></div></td><td className="px-6 py-4"><span className="rounded-full bg-emerald-50 px-2 py-1 text-xs font-black uppercase tracking-wider text-emerald-700">{tx.status}</span></td><td className={clsx('px-6 py-4 text-right font-black',tx.type==='Earning'?'text-emerald-600':'text-slate-900')}>{tx.type==='Earning'?'+':'-'}${tx.amount.toFixed(2)}</td><td className="px-6 py-4 text-right"><button type="button" onClick={()=>setDetail(tx)} className="text-[10px] font-black uppercase tracking-widest text-primary hover:underline">Details</button></td></tr>)}
